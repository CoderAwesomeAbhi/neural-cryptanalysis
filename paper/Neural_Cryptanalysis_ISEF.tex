...
\affil{\normalsize UT Austin Research Assistant Program $\cdot$ ISEF\\[-2pt]
...
\begin{abstract}
We study piecewise affine recurrences $x_{n+1} = A_{\varphi(x_n)}\,x_n + b_{\varphi(x_n)} \pmod{m}$ over residue rings $\Z/p^k\Z$, where the transition matrix is selected by a nonlinear switching function $\varphi$. The central algebraic parameter is the \emph{Hensel index} $\delta_i = \vp(\det(A_i - \Id))$: when $\delta_i = 0$ for all regime matrices (Hensel-satisfied), the maximum period grows super-linearly with the extension level $k$.

We make five contributions. \textbf{(1) Two proven theorems and two observations.} We prove super-linear growth ($T(p^{k+1}) > p \cdot T(p^k)$ for all $k \geq 1$) with a complete rigorous proof via orbit counting and regime boundary analysis. We prove an explicit period lower bound ($T(p^k) \geq p^{k-1}(p-1)r$). We document an empirical neural hardness threshold ($T_{\mathrm{critical}} \approx N_{\mathrm{train}}/25$) and test the $p$-adic attention hypothesis with $L_{\mathrm{in}}=32$, finding no significant correlation (negative result). \textbf{(2) Cross-prime validation.} We compute maximum periods for primes $p \in \{5, 7, 11, 13\}$ at extension levels $k = 1, 2, 3$ using the same structural matrices. The Hensel-satisfied configuration outperforms the violated one at every data point; the sat/viol period ratio at $k=2$ ranges from $7.1\times$ to $135.6\times$. \textbf{(3) Algebraic attack analysis.} We analyze polynomial system solving, CRT decomposition, and lattice-based attacks, showing that prime power moduli with $m \geq 100$ provide high resistance. \textbf{(4) Rigorous statistical analysis.} All neural results include bootstrap confidence intervals, hypothesis tests with Bonferroni correction, and effect size calculations. \textbf{(5) Phase transition characterization.} MLP, LSTM, and Transformer models achieve $100\%$ accuracy when $T/L_{\mathrm{in}} \leq 21$ and collapse to $\leq 16.3\%$ when $T/L_{\mathrm{in}} \geq 181$. The transition lies in $(21, 181)$.

All code is open-source and fully reproducible. No cryptographic security claims are made.
\end{abstract}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Observation~\ref{obs:padic}:} $p$-adic attention hypothesis tested with $L_{\mathrm{in}}=32$. Result: No significant correlation detected (negative result).
\end{itemize}
...
\begin{observation}[$p$-adic Attention Hypothesis - Negative Result]
\label{obs:padic}
...
\textbf{Result:} Mean Spearman correlation $\rho = 0.026$ with only 1/32 query positions showing significant correlation ($p < 0.05$). The hypothesis is \textbf{not supported} by the data.
