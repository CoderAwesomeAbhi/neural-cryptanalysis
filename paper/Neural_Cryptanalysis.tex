...
Let $f$ be the piecewise affine map~\eqref{eq:map} with Hensel-satisfied matrices ($\delta(A_i) = 0$ for all $i$). Then the extension $\tilde{f}\colon \Z_p^2 \to \Z_p^2$ is a measure-preserving transformation with respect to the product Haar measure $\mu_p \times \mu_p$.
\end{theorem}
...
\begin{observation}[Ergodic Unpredictability]
...
% ════════════════════════════════════════════════════════════════════════════
\section{Theoretical Derivation of the Neural Resistance Threshold}
\label{sec:neural_theory}

We now derive a theoretical formula for the critical threshold $T_{\mathrm{crit}}$ at which neural networks fail, connecting it to the VC dimension and Rademacher complexity of the hypothesis class.
...
% ════════════════════════════════════════════════════════════════════════════
\section{$p$-adic Positional Encoding: Fixing the Transformer}
\label{sec:padic_encoding}

Motivated by the weak attention--$p$-adic correlation in Observation~\ref{obs:padic}, we outline an architectural modification that can be tested in future experiments.

\subsection{Standard positional encoding}

The standard Transformer uses sinusoidal positional encodings~\citep{Vaswani2017}:
\begin{equation}
\mathrm{PE}(pos, 2i) = \sin(pos / 10000^{2i/d}), \quad \mathrm{PE}(pos, 2i+1) = \cos(pos / 10000^{2i/d})
\end{equation}
where $pos$ is the position index and $i \in \{0, \ldots, d/2-1\}$ indexes the embedding dimension.

These encodings capture \emph{linear} positional relationships: positions $i$ and $j$ with $|i-j|$ small have similar encodings. However, they do not capture $p$-adic distance: positions $i$ and $j$ with $\vp(i-j)$ large (i.e., $i \equiv j \pmod{p^k}$ for large $k$) have unrelated encodings.

\subsection{$p$-adic positional encoding}

We propose a new encoding that respects $p$-adic distance:

\begin{definition}[$p$-adic Positional Encoding]
For a sequence over $\Z/p^k\Z$ with prime $p$, define the $p$-adic positional encoding as:
\begin{equation}
\mathrm{PE}_p(pos, i) = \sin\left(\frac{pos \bmod p^{i+1}}{p^{i+1}} \cdot 2\pi\right)
\end{equation}
for $i \in \{0, \ldots, k-1\}$.
\end{definition}

\textbf{Key property:} Positions $i$ and $j$ with $\vp(i-j) = \ell$ have identical encodings in the first $\ell$ dimensions:
\begin{equation}
\mathrm{PE}_p(i, m) = \mathrm{PE}_p(j, m) \quad \text{for all } m < \ell.
\end{equation}

This encodes the $p$-adic tree structure: positions that share a common ancestor in the lifting tree have similar encodings.

\subsection{Hybrid encoding}

To capture both linear and $p$-adic structure, we propose a hybrid encoding:
\begin{equation}
\mathrm{PE}_{\mathrm{hybrid}}(pos) = \alpha \cdot \mathrm{PE}_{\mathrm{sin}}(pos) + (1-\alpha) \cdot \mathrm{PE}_p(pos)
\end{equation}
where $\alpha \in [0,1]$ is a learnable parameter that balances linear and $p$-adic information.

\subsection{Evaluation protocol (planned and partially implemented)}

\textbf{Implementation status:} We implemented prototype code for three Transformer variants with different positional encodings and training on the hard sequence ($p=5$, $k=3$, $m=125$, $T=7295$) at context window $L_{\mathrm{in}}=32$.

\textbf{Experimental design:}
\begin{enumerate}[leftmargin=2em]
\item Train three Transformers on the $p=5$, $m=125$, $T=7295$ sequence:
  \begin{itemize}
  \item Baseline: standard sinusoidal encoding
  \item $p$-adic: pure $p$-adic encoding with $p=5$
  \item Hybrid: learnable combination with $\alpha$ initialized to 0.5
  \end{itemize}
\item Use $L_{\mathrm{in}} = 32$ (longer context to capture $p$-adic structure)
\item Train for 100 epochs with Adam optimizer (lr=0.001)
\item Measure: (a) validation accuracy, (b) convergence speed
\end{enumerate}

\textbf{Current evidence:} The positional-encoding variants are implemented, but a full controlled benchmark (multiple seeds, confidence intervals, and reproducible figure artifacts) remains ongoing. Therefore, we treat this section as a concrete research direction rather than a finalized claim.

\textbf{If validated in the full benchmark, implications would be:}
\begin{enumerate}[leftmargin=2em]
\item The Transformer failure is due to \emph{positional encoding}, not attention mechanism
\item $p$-adic structure \emph{can} be learned with appropriate inductive bias
\item Modern LLMs could be improved for mathematical reasoning by incorporating $p$-adic encodings
\end{enumerate}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Number theory:} Sequences defined by recurrences over $\Z/p^k\Z$
\item \textbf{Hierarchical data:} Tree-structured data with branching factor $p$
\item \textbf{Modular arithmetic:} Any problem involving congruences modulo prime powers
\end{itemize}
