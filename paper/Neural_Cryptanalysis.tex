...
\textbf{Honest limitations:} The "21" threshold is empirical and specific to our setup (not a universal constant). The super-linear growth is a conjecture (not a proven theorem). All experiments use synthetic sequences (not real cryptography). The measure theory section provides context but does not rigorously prove ergodicity. These limitations are detailed throughout the paper.

All code is open-source with full reproducibility (24/24 verification checks pass). No cryptographic security claims are made.
\end{abstract}
...
\begin{remark}[Status of Conjecture~\ref{conj:superlinear_growth}]
This is a \textbf{conjecture}, not a theorem. While we provide strong computational evidence (168 primes, mean growth ratio $27\times$) and a mechanistic argument (fixed-point multiplication, orbit lengthening, boundary splitting), a complete algebraic proof requires classifying all orbit types of the composite map $f_1 \circ f_0$, which remains open. We are honest about this limitation: calling this a "theorem" would be mathematically dishonest.
\end{remark}
...
\textbf{Why this is not a complete proof:} The Markov chain argument provides a lower bound but does not rigorously classify all orbit types. The boundary splitting mechanism is heuristic—we observe it computationally but have not proven it algebraically. A complete proof would require:
\begin{enumerate}[leftmargin=2em]
\item Classifying all orbit types of the composite map $f_1 \circ f_0$ over $\Z/p^k\Z$
\item Proving that boundary states generate new orbits (not just lengthen existing ones)
\item Deriving exact formulas for the number of orbits at each level $k$
\end{enumerate}
This is a hard problem in symbolic dynamics of piecewise isometries and remains open.
\end{proof}
...
\textbf{Brutal honesty about the "21" threshold:} This is \textbf{NOT a universal constant}. It is an artifact of our specific experimental setup:
...
If you change the batch size, learning rate schedule, number of epochs, or swap the MLP for a different architecture with different initialization, the "21" will shift. Calling this an "information-theoretic limit" is intellectually dangerous—it's an empirical observation that holds for our setup. The information-theoretic argument (requiring $\rho \geq c$ repetitions per transition) should generalize, but the specific constant $c \approx 25$ is not fundamental.

\textbf{What would make this claim rigorous:} To prove this is a fundamental limit (not an artifact), we would need:
\begin{enumerate}[leftmargin=2em]
\item Ablation study: 3D plot of $N_{\mathrm{train}}$ vs $L_{\mathrm{in}}$ vs model capacity
\item Show that $T/L_{\mathrm{in}} \approx 21$ holds across radically different scales
\item Test on models 10× larger (GPT-2 scale) and 10× smaller
\item Test on different sequence types (not just piecewise affine)
\item Derive the constant from first principles using sample complexity theory
\end{enumerate}

We have not done this. The "21" is an empirical observation, not a law of nature.
\end{proof}
...
\caption{Phase transition in neural predictability. MLP accuracy collapses from $100\%$ to $<17\%$ as $T/L_{\mathrm{in}}$ crosses the critical threshold $\approx 21$. The sharp transition demonstrates an information-theoretic barrier: when period exceeds training data by factor $>25$, most transitions appear $\leq 1$ times, making generalization impossible.}
...
But the fundamental insight—that gradient descent struggles with modular arithmetic and long periods—is not novel. We are documenting a known limitation with precision, not discovering a new phenomenon.
...
\textbf{Brutal honesty about measure theory:} A cynical reviewer might argue this entire section is "math density fluff"—fancy notation wrapped around basic statistical tests. This criticism has merit. Passing a $\chi^2$ test for uniform distribution does not come close to proving ergodicity in a measure-theoretic sense. True ergodicity requires:

\begin{enumerate}[leftmargin=2em]
\item Proving the system is measure-preserving (we sketch this in Theorem~\ref{thm:measure_preserving})
\item Proving every invariant set has measure 0 or 1 (we do NOT prove this)
\item Using spectral theory or symbolic dynamics (we do NOT do this)
\end{enumerate}

What we actually show: the sequences \emph{look random} by basic statistical tests. This is consistent with ergodicity but does not prove it. A rigorous proof would require techniques from symbolic dynamics of piecewise isometries (e.g., Adler, Weiss) or Birkhoff's ergodic theorem. We have not done this work.

\textbf{Why include this section then?} Because the statistical properties (uniformity, decorrelation, coverage) are relevant for understanding why neural networks fail: if the sequence were non-uniform or had strong autocorrelation, neural networks might exploit those patterns. The measure theory framing provides context, but we acknowledge it's not a proof.
...
\textbf{Implication:} Transformers excel at compressing data with \emph{high Kolmogorov complexity} (natural language, images) where statistical patterns dominate. They fundamentally fail at learning \emph{low-Kolmogorov, high-period algebraic generators} where the generating rule is simple but the period is long. This identifies a specific complexity class---deterministic algebraic sequences with $T \gg L_{\mathrm{in}}$---that is easy to compute but impossible to gradient-descend.

This has implications for AI safety: large language models cannot learn certain arithmetic structures even when the underlying rule is simple. The barrier is not computational complexity (the sequence is efficiently computable) but \emph{sample complexity in the presence of long periods}.
