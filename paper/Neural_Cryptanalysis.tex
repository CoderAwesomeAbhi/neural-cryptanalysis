...
\midrule
Easy ($m=25$, $T=125$) & 100.0\% & 100.0\% & 100.0\%\\
Hard ($m=125$, $T=7295$) & 1.3\% & 1.2\% & 2.6\%\\
\bottomrule
\end{tabular}
\end{center}

\textbf{Conclusion:} SSMs fail identically to Transformers on hard sequences. This rules out ``insufficient memory'' as the cause of failure and confirms the barrier is \emph{information-theoretic, not architectural}. When $T \gg N_{\mathrm{train}}$, no architecture can generalize because most transitions appear $\leq 1$ time in training.

\subsection{Optimal Prediction Bound (Theorem 11.1)}
\label{sec:results:optimal}

We verify the information-theoretic upper bound computationally. For any predictor $f\colon \Sigma^{L_{\mathrm{in}}} \to \Sigma$ trained on $N$ consecutive transitions of a period-$T$ sequence with $N < T$:
\begin{equation}
...
p5k3sat  & 125 & 7295  & 49.7\% & 49.3\% & 2.6\%\\
p7k2sat  & 49  & 1083  & 83.5\% & 82.8\% & 16.3\%\\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{LC-NC Separation (Theorem 12.1)}
\label{sec:results:separation}

We prove Linear Complexity (LC) and Neural Complexity (NC) are \emph{independent} measures by constructing explicit sequences with:
\begin{itemize}
\item \textbf{High LC + Low NC:} H-viol $m=25$ has $\LC=27$ but neural accuracy $=100\%$ (period $T=30$ is short)
\item \textbf{Low LC + High NC:} LFSR with $\LC=3$ but $T=124$ gives neural accuracy $<10\%$
\end{itemize}
...
\subsection{Computational ergodicity verification}
\label{sec:ergodicity_verification}

For deterministic periodic maps, classical spectral gap analysis fails (all eigenvalues lie on the unit circle). We instead measure ergodicity via four computational tests:
...
This provides \textbf{computational support} for Conjecture~\ref{conj:ergodic}: Hensel-satisfied maps behave ergodically in all measurable senses. A formal proof remains open (listed as Future Work).
