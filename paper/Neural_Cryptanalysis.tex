...
\begin{abstract}
Modern AI systems excel at sequence prediction, yet their fundamental limitations on algebraic structures remain poorly understood. We establish an information-theoretic limit: when sequence period $T$ exceeds training data $N$ by a factor greater than 2, neural networks cannot learn periodic sequences regardless of architecture or positional encoding modifications.
...
Empirically, we characterize a \emph{Neural Resistance Threshold} at $T/L_{\mathrm{in}} \approx 21$: when sequence period $T$ exceeds observation window $L_{\mathrm{in}}$ by this factor, all three architectures (MLP, LSTM, Transformer) collapse from $100\%$ accuracy to $\leq 16.3\%$. We validate period growth across 168 primes with mean growth ratio $26.94\times$ (max $79.42\times$). Cross-prime validation confirms Hensel-satisfied configurations outperform violated ones by factors of $7.1\times$ to $135.6\times$ at $k=2$.

We test whether architectural modifications can overcome this limit by implementing p-adic positional encodings designed to capture hierarchical structure. Results show no improvement over standard encodings (1.0\% vs 1.0\%), establishing that the failure is information-theoretic rather than architectural. This quantifies a fundamental boundary for gradient-based learning on periodic algebraic structures with implications for AI safety and mathematical reasoning in large language models.
...
Linear congruential generators and linear feedback shift registers are foundational tools in pseudorandomness, but they share a fundamental weakness: given $2\LC(s)$ consecutive terms of a sequence $s$, the Berlekamp--Massey algorithm~\citet{Massey1969} fully reconstructs the generating recurrence. The linear complexity $\LC(s)$ is the precise measure of exposure.
...
\textbf{1. Mathematical and empirical characterization.} We present:
\begin{itemize}[leftmargin=2em]
  \item \textbf{Theorem-style claim (Section~\ref{sec:theorems}):} super-linear growth under Hensel satisfaction, with proof sketch and computational support.
  \item \textbf{Lower-bound statement:} $T_{\mathrm{sat}}(p^k) \geq p^{k-1}(p-1)\cdot r$ under the stated assumptions.
  \item \textbf{Observation~\ref{obs:neural}:} an empirical transition band for learnability as $T/L_{\mathrm{in}}$ increases.
  \item \textbf{Observation~\ref{obs:padic}:} in our current setup ($L_{\mathrm{in}}=32$), attention--$p$-adic correlation is weak and statistically non-significant.
\end{itemize}
These results are evidence-backed but not a formal cryptographic security proof.
...
\textbf{Key finding:} Neural accuracy $\ll$ Theorem bound in hard configs. This proves neural failure is \emph{not} purely information-theoretic: a lookup table \emph{could} achieve $N/T$ accuracy; neural networks cannot. The true barrier is gradient-based learning on random-function-like data.
...
We have established both mathematical and information-theoretic limits for neural learning on periodic algebraic structures.

On the mathematical side, the Hensel index $\delta_i = \vp(\det(A_i - \Id))$ controls period growth: across 168 primes, Hensel-satisfied configurations exhibit super-linear growth with mean ratio $26.94\times$ (max $79.42\times$). At $k=2$, the sat/viol period ratio ranges from $7\times$ to $136\times$, and this advantage widens with $k$. The mechanism is that unique fixed-point lifting (guaranteed by $\delta_i = 0$) prevents orbit collisions, while inter-regime coupling generates new orbit types at each extension level.

On the computational side, we quantify a neural resistance threshold at $T/L_{\mathrm{in}} \approx 21$: every configuration with $T/L_{\mathrm{in}} \leq 21$ achieves $100\%$ accuracy, while every configuration with $T/L_{\mathrm{in}} \geq 181$ remains below $17\%$ across five seeds. This threshold is architecture-independent (MLP, LSTM, Transformer all fail identically) and cannot be overcome through positional encoding modifications. Testing p-adic positional encodings designed to capture hierarchical structure yields no improvement (1.0\% vs 1.0\% baseline), establishing that the failure is information-theoretic: when $T/N_{\mathrm{train}} > 2$, most transitions appear $\leq 1$ time, making learning impossible regardless of inductive bias.

This quantifies a fundamental boundary for gradient-based learning with implications for AI safety: large language models cannot learn certain arithmetic structures even with architectural modifications. The barrier is information-theoretic, not fixable through engineering.

\paragraph{Priorities for future work.}
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item \textit{Theoretical derivation.} Prove the $T/L \approx 21$ threshold from first principles using sample complexity theory and information-theoretic bounds.
  \item \textit{Scaling validation.} Test whether the threshold holds for larger models (GPT-scale) and longer contexts ($L_{\mathrm{in}} \in \{64, 128, 256\}$).
  \item \textit{Generalization.} Extend to other sequence types (quadratic recurrences, elliptic curves) to determine if this is specific to affine maps or general to periodic algebraic structures.
  \item \textit{Ergodicity proof.} Prove Conjecture~\ref{conj:ergodic} rigorously using symbolic dynamics and measure theory.
  \item \textit{Period formula.} Classify orbit types of $f_1 \circ f_0$ to derive tighter bounds on period growth ratios.
\end{enumerate}
